...
#~ 2- إن كان غير ثلاثي
    ...
    # deprecated
    def _prepare_derivation_stem2222(self):
        """
        Prepare the derivation stems 
         Those stems will be concatenated with conjugation affixes.
          This function store results in self.tab_conjug_stem. 
...
#~ ل
    ...
    # deprecated
    def _prepare_derivation_stem22222222(self):
        """
        Prepare the derivation stems 
         Those stems will be concatenated with conjugation affixes.
          This function store results in self.tab_conjug_stem. 
        ...
        # الفعل الرباعي
            ...
            derivation_subject_marks = FATHA + SUKUN + KASRA + DAMMA
            derivation_object_marks  = FATHA + SUKUN + FATHA + DAMMA
        # الفعل الخماسي
        elif self.vlength == 5:
            if letters.startswith(TEH):
                # الفعل الخماسي المبدوء بتاء يختلف في حركة عين اسم الفاعل عن مضارعه
                derivation_subject_marks = FATHA + FATHA + SUKUN + KASRA + DAMMA
                derivation_object_marks  = FATHA + FATHA + SUKUN + FATHA + DAMMA
            else :
    ...
    # معالجة الألفات في الفعل والحركات الطويلة
        ...
        if letters.startswith(ALEF) or self.hamza_zaida:
            ...
            derivation_subject_letters =  letters[1:]
            derivation_object_letters =   letters[1:]              
        ...
        if self.vlength == 3 :
            if  len(self.word_letters) == 2:
                derivation_subject_letters =  letters[0] + HAMZA + letters[1]
                derivation_object_marks =  self.future_type + FATHA  
                derivation_object_letters =  letters
            ...
            self.word_letters.endswith(ALEF) or self.word_letters.endswith(YEH)):
                derivation_object_marks   = SUKUN + self.future_type + FATHA    #+ DAMMA
                derivation_object_letters =  derivation_object_letters  #+ SHADDA
                
        # معالجة الفعل الناقص عند تصريفه في المجهول
        # تستبدل واو التاقص الذي حركة عين ماضيه فتحة بياء
##        passive_letters=future_letters
        if self.vlength == 3 and derivation_object_letters.endswith(vconst.ALEF_MAMDUDA):
