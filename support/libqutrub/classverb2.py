...
# نقوم بتحويل الحركة الطويلة إلى ما يوافقها
        if len(marks) < len(new_marks):
            ...
            if alef_haraka_pos >= 0 and alef_haraka_pos + 1 < len(new_marks):
                ...
                #~ changed_haraka = vconst.HOMOGENIZE_ALEF_HARAKA_TABLE[first][second]
                if vconst.HOMOGENIZE_ALEF_HARAKA_TABLE.has_key(first) and vconst.HOMOGENIZE_ALEF_HARAKA_TABLE[first].has_key(second):
                    changed_haraka = vconst.HOMOGENIZE_ALEF_HARAKA_TABLE[first][second]
...
#####################################
    def _prepare_future_imperative_stem(self):
        ...
        if self.vlength == 3:
            ...
            derivation_object_marks  = SUKUN + vconst.WAW_HARAKA + DAMMA            
        ...
        # الفعل الرباعي
            ...
            derivation_subject_marks = future_marks
            derivation_object_marks  = passive_future_marks
        # الفعل الخماسي
        elif self.vlength == 5:
            first_future_mark = FATHA
            first_passive_future_mark = DAMMA
            if letters.startswith(TEH):
                ...
                derivation_subject_marks = DAMMA + SUKUN + FATHA + KASRA + DAMMA
                derivation_object_marks  = DAMMA + SUKUN + FATHA + FATHA + DAMMA
        ...
        #الفعل السداسي
            ...
            derivation_subject_marks = DAMMA + SUKUN + FATHA + SUKUN + KASRA + DAMMA
            derivation_object_marks  = DAMMA + SUKUN + FATHA + SUKUN + FATHA + DAMMA
        else:
            # to handle errors
            # The KASRA is not correct, we use this problem to detect bugs
            first_future_mark = KASRA
            first_passive_future_mark = KASRA
            future_marks = KASRA* self.vlength
            passive_future_marks = KASRA * self.vlength

            derivation_subject_marks = KASRA * self.vlength
            derivation_object_marks  = KASRA * self.vlength
    ...
    # معالجة الألفات في الفعل والحركات الطويلة
        ...
        if letters.startswith(ALEF) or self.hamza_zaida:
            ...
            derivation_subject_marks = derivation_subject_marks[1:]
            derivation_object_marks = derivation_object_marks[1:]
            derivation_subject_letters =  future_letters
            derivation_object_letters =   future_letters              
          
        # حالة الفعل المثال
        elif (self.vlength == 3 and self.word_letters.startswith(WAW) and 
        (self.future_type == KASRA or 
         (self.future_type==FATHA and  self.word_marks==FATHA+FATHA+FATHA and \
...
##            passive_future_marks=passive_future_marks[1:]
            ...
            derivation_subject_letters =  letters
            derivation_object_letters =   letters   
            # حالة الفعل اﻷجوف في اشتقاق اسم الفاعل
        if self.vlength == 3 :
            if  len(self.word_letters) == 2:
                derivation_subject_letters =  letters[0] + HAMZA + letters[1]
                derivation_object_marks =  future_marks  
                derivation_object_letters =  future_letters
            elif self.word_letters.endswith(SHADDA):
            ...
            self.word_letters.endswith(ALEF) or self.word_letters.endswith(YEH)):
                derivation_object_marks =  future_marks   +DAMMA
                derivation_object_letters =  future_letters  + SHADDA
...
##        passive_letters=future_letters
        if self.vlength == 3 and passive_letters.endswith(vconst.ALEF_MAMDUDA):
            ...
            derivation_subject_letters =  derivation_subject_letters[:-1]+YEH
            derivation_object_letters =   derivation_object_letters[:-1]+ALEF_MAKSURA              
            
        if self.vlength !=3 :
            derivation_subject_marks = DAMMA + derivation_subject_marks
            derivation_object_marks = DAMMA + derivation_object_marks
        else:
            derivation_subject_marks = derivation_subject_marks
            derivation_object_marks = FATHA + derivation_object_marks
        ...
        self.tab_conjug_stem[vconst.SubjectNoun] = ConjugStem(
        vconst.SubjectNoun, derivation_subject_letters, derivation_subject_marks)
        #~ vconst.SubjectNoun, future_letters, derivation_subject_marks)
        ### اشتقاق اسم الفاعل والمفعول
        self.tab_conjug_stem[vconst.ObjectNoun] = ConjugStem(
        vconst.ObjectNoun, derivation_object_letters, derivation_object_marks)
        #~ vconst.ObjectNoun, future_letters, derivation_object_marks)
...
##        return self.join(stem_l, stem_m, prefix, suffix)
    ...
    def derivate(self):
        ...
        @rtype: unicode;        
        """
        subj = self.conjugate_tense_pronoun(vconst.SubjectNoun, u"")
        obj = self.conjugate_tense_pronoun(vconst.ObjectNoun, u"")
...
# تحضير جذوع التصريف في المضارع والأمر للأفعال الضاذة
    def _prepare_irregular_future_imperative_stem(self):
        ...
        if self._is_irregular_verb():
            ...
            # اشتقاق اسم الفاعل والمفعول
            self.tab_conjug_stem[vconst.SubjectNoun] = ConjugStem(
                        vconst.SubjectNoun, self.word_letters, vconst.ALEF_HARAKA + KASRA + DAMMA)
            self.tab_conjug_stem[vconst.ObjectNoun] = ConjugStem(
                        vconst.ObjectNoun, self.word_letters, FATHA+ SUKUN +vconst.WAW_HARAKA + DAMMA)                   
        return False
